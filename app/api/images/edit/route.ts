...
async function editWithOpenAI(asset:Asset,prompt:string,mask:string):Promise<Generated>{
  const key=process.env.OPENAI_API_KEY;if(!key)throw new Error('服务器尚未配置 GPT Image API 密钥，请联系管理员');
  const sourceDir=process.env.ASSET_DATA_DIR||path.join(process.cwd(),'data');const source=await readFile(path.join(sourceDir,'assets',path.basename(asset.storageKey)));
  const parsedMask=dataUrl(mask,'修改遮罩',5*1024*1024);const form=new FormData();
  form.append('model',process.env.OPENAI_IMAGE_MODEL||'gpt-image-2');form.append('prompt',prompt);form.append('image[]',new Blob([source],{type:asset.mimeType}),'original.'+extension(asset.mimeType));form.append('mask',new Blob([Buffer.from(parsedMask.base64,'base64')],{type:'image/png'}),'edit-mask.png');form.append('size','1024x1024');form.append('quality','high');form.append('output_format','png');
  const response=await fetch(`${providerBaseUrl('OPENAI_BASE_URL','https://api.openai.com/v1')}/images/edits`,{method:'POST',headers:{Authorization:`Bearer ${key}`},body:form,signal:AbortSignal.timeout(280_000)});
  const data=await response.json() as {data?:Array<{b64_json?:string}>;error?:{message?:string}};if(!response.ok||!data.data?.[0]?.b64_json)throw new Error(data.error?.message||`GPT Image 未返回编辑结果（HTTP ${response.status}）`);return {base64:data.data[0].b64_json,mimeType:'image/png',model:process.env.OPENAI_IMAGE_MODEL||'gpt-image-2'};
}

async function editWithGemini(asset:Asset,prompt:string,mask:string):Promise<Generated>{
  const key=process.env.GEMINI_API_KEY;if(!key)throw new Error('服务器尚未配置 Nano Banana API 密钥，请联系管理员');
  const sourceDir=process.env.ASSET_DATA_DIR||path.join(process.cwd(),'data');const source=await readFile(path.join(sourceDir,'assets',path.basename(asset.storageKey)));const parsedMask=dataUrl(mask,'修改遮罩',5*1024*1024);
  ...
  const response=await fetch(`${providerBaseUrl('GEMINI_BASE_URL','https://generativelanguage.googleapis.com/v1beta')}/interactions`,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':key},body:JSON.stringify({model:process.env.GEMINI_IMAGE_MODEL||'gemini-3.1-flash-image',input,response_format:{type:'image',aspect_ratio:'1:1',image_size:'1K'}}),signal:AbortSignal.timeout(280_000)});
  const data=await response.json() as {output_image?:{data?:string;mime_type?:string};error?:{message?:string}};if(!response.ok||!data.output_image?.data)throw new Error(data.error?.message||`Nano Banana 未返回编辑结果（HTTP ${response.status}）`);return {base64:data.output_image.data,mimeType:data.output_image.mime_type||'image/png',model:process.env.GEMINI_IMAGE_MODEL||'gemini-3.1-flash-image'};
}
...
export async function POST(request:NextRequest){
  ...
  const provider:Provider=body.provider==='openai'?'openai':'gemini';const creditCost=provider==='openai'?10:6;const configured=provider==='openai'?Boolean(process.env.OPENAI_API_KEY):Boolean(process.env.GEMINI_API_KEY);if(!configured)return NextResponse.json({error:`${provider==='openai'?'GPT Image':'Nano Banana'} 未配置图像编辑能力，请联系管理员`},{status:422});
  const reserved=await db()<Array<{credits:number}>>`UPDATE app_users SET credits=credits-${creditCost} WHERE id=${user.id} AND credits>=${creditCost} RETURNING credits`;if(!reserved[0])return NextResponse.json({error:`算力不足，本次需要 ${creditCost} 点`},{status:402});
  try{const prompt=buildPrompt(asset,body);const generated=provider==='openai'?await editWithOpenAI(asset,prompt,body.mask):await editWithGemini(asset,prompt,body.mask);const assetId=randomUUID();const storageKey=`${assetId}.${extension(generated.mimeType)}`;const dataDir=process.env.ASSET_DATA_DIR||path.join(process.cwd(),'data');await mkdir(path.join(dataDir,'assets'),{recursive:true});await writeFile(path.join(dataDir,'assets',storageKey),Buffer.from(generated.base64,'base64'),{mode:0o600});const root=asset.rootAssetId||asset.id;const [next]=await db()<Array<{version:number}>>`SELECT coalesce(max(version_number),0)::int+1 AS version FROM generated_assets WHERE root_asset_id=${root}`;await db()`UPDATE generated_assets SET is_current=false WHERE root_asset_id=${root}`;await db()`INSERT INTO generated_assets(id,owner_id,project_id,provider,model,slot,prompt,storage_key,mime_type,credit_cost,parent_asset_id,root_asset_id,version_number,is_current,edit_metadata) VALUES(${assetId},${user.id},${asset.projectId},${provider},${generated.model},${asset.slot},${prompt},${storageKey},${generated.mimeType},${creditCost},${asset.id},${root},${next.version},true,${JSON.stringify({instruction:body.instruction||'',regions:body.regions||[],mode:body.mode||'local'})}::jsonb)`;await db()`UPDATE projects SET updated_at=now(),status='review',image_count=image_count+1 WHERE id=${asset.projectId}`;return NextResponse.json({assetId,imageUrl:`/api/assets/${assetId}`,versionNumber:next.version,creditsRemaining:reserved[0].credits});
  }catch(error){await db()`UPDATE app_users SET credits=credits+${creditCost} WHERE id=${user.id}`;console.error('NovaCanvas image edit failed',error);return NextResponse.json({error:error instanceof Error?error.message:'图片修改失败'},{status:503});}
}
