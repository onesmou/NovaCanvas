...
import { saveGeneratedAsset } from '../../../../lib/image-storage';
...
async function generateWithGemini(body:Body,config:ProviderConfig|null):Promise<Generated>{
  const key=config?.apiKey||process.env.GEMINI_API_KEY;if(!key)throw new Error('服务器尚未配置 Nano Banana API 密钥，请联系管理员');
  const model=config?.model||process.env.GEMINI_IMAGE_MODEL||'gemini-3.1-flash-image';
  ...
  const baseUrl=config?.baseUrl||providerBaseUrl('GEMINI_BASE_URL','https://generativelanguage.googleapis.com/v1beta');
...
async function generateWithOpenAI(body:Body,config:ProviderConfig|null):Promise<Generated>{
  ...
  const baseUrl=config?.baseUrl||providerBaseUrl('OPENAI_BASE_URL','https://api.openai.com/v1');let response:Response;
