...
export async function GET(request:NextRequest,{params}:{params:Promise<{key:string[]}>}){
  ...
  try{const dataDir=process.env.ASSET_DATA_DIR||path.join(process.cwd(),'data');const data=await readFile(path.join(dataDir,'assets',safeName));const download=request.nextUrl.searchParams.get('download')==='1';return new NextResponse(data,{headers:{'Content-Type':asset.mimeType,'Cache-Control':'private, max-age=31536000, immutable','Content-Disposition':`${download?'attachment':'inline'}; filename="${safeName}"`}});}catch{return NextResponse.json({error:'素材文件暂时不可用'},{status:404});}
}
