...
export default function Home() {
  const [activeNav,setActiveNav]=useState('套图工作台');
  ...
  useEffect(()=>{fetch('/api/projects').then(r=>r.ok?r.json():null).then(data=>{const payload=data as {projects?:Array<{projects:ProjectRow}>}|null;const rows=(payload?.projects??[]).map(item=>item.projects);setProjectRows(rows)}).catch(()=>setProjectRows([]))},[generatedImage]);
  ...
  function usePrompt(p:PromptCard){setSelected(p);setPrompt(`${p.prompt}\n\n品类：${category}；站点：Amazon ${market}。\n负面约束：${p.negative}`);notify(`已载入「${p.title}」`)}
  async function generate(){
    setRunning(true); setGeneratedImage(null); notify(`正在调用 ${provider==='gemini'?'Nano Banana':'GPT Image'}…`);
    try{
      const response=await fetch('/api/images/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider,prompt,referenceImage:productImage,size:selected.id==='a-hero'?'1536x1024':'1024x1024',projectId,projectTitle,slot:selected.slot})});
  ...
  return <main className="amazon-app">
    <aside className="amazon-sidebar">
      ...
      <nav>
        <p>生产中心</p>
        {['套图工作台','提示词库','A+ 内容画布','合规检测','Listing 项目'].map((n,i)=><button className={activeNav===n?'active':''} onClick={()=>setActiveNav(n)} key={n}><span>{['▦','✦','A+','✓','▤'][i]}</span>{n}{n==='提示词库'&&<em>12</em>}</button>)}
        <p>团队管理</p>
        <Link href="/account"><span>◎</span>账号与套餐</Link>
        <Link href="/admin"><span>⌘</span>管理后台</Link>
      </nav>
    ...
    <section className="amazon-main">
      <header className="amazon-topbar">
        ...
        <div className="top-actions"><button>帮助中心</button><button className="bell">⌁</button><Link href="/account" className="user-chip"><i>JE</i><span><b>JinYuu Team</b><small>管理员</small></span><em>⌄</em></Link></div>
      </header>
        ...
        <section className="production-grid">
          <div className="storyboard-card">
            ...
            <div className="storyboard-grid">
              {promptLibrary.slice(0,7).map((p,i)=><button key={p.id} onClick={()=>usePrompt(p)} className={selected.id===p.id?'selected':''}><div className={`board-art b${i}`}>{productImage&&<img src={productImage} alt=""/>}<span>{p.slot}</span><i>{i===0?'MAIN':i===2?'IN USE':i===5?'IN THE BOX':'IMAGE'}</i></div><b>{p.title}</b><small>{p.goal}</small><em>{selected.id===p.id?'正在编辑':'编辑提示词'}</em></button>)}
            </div>
