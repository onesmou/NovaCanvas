...
export async function ensureSelfHostedSchema() {
  ...
  await sql`ALTER TABLE generated_assets ADD COLUMN IF NOT EXISTS parent_asset_id uuid REFERENCES generated_assets(id) ON DELETE SET NULL`;
  await sql`ALTER TABLE generated_assets ADD COLUMN IF NOT EXISTS root_asset_id uuid`;
  await sql`ALTER TABLE generated_assets ADD COLUMN IF NOT EXISTS version_number integer NOT NULL DEFAULT 1`;
