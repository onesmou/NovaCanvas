...
async function passwordMatches(password:string, stored:string) { const [,salt,hash] = stored.split('$'); if (!salt || !hash) return false; const actual = await passwordHash(password, salt); return timingSafeEqual(Buffer.from(actual), Buffer.from(stored)); }
